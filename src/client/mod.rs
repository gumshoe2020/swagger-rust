...
impl<C> Api<C> for Client where C: Has<XSpanIdString> + Has<Option<AuthData>>{
    ...
    fn delete_order(&self, param_order_id: i64, context: &C) -> Box<Future<Item=DeleteOrderResponse, Error=ApiError>> {


        let uri = format!(
            "{}/v2/store/order/{orderId}",
            self.base_path, orderId=utf8_percent_encode(&param_order_id.to_string(), PATH_SEGMENT_ENCODE_SET)
        );
            ...
            match response.status().as_u16() {
                200 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            LoginUserResponse::SuccessfulOperation{ body: body, x_expires_after: response_x_expires_after, x_rate_limit: response_x_rate_limit }
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                400 => {
